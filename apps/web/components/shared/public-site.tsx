...
export function PublicFooter() {
  return (
    <footer style={{ background: "var(--page-bg2)", borderTop: "1px solid var(--line)" }}>
      <div className="mx-auto grid max-w-7xl gap-12 px-5 py-16 lg:grid-cols-[1.4fr_1fr_1fr_1.2fr] lg:px-8">
        <div>
          ...
          <a
            ...
            className="mt-5 inline-flex items-center gap-2 text-sm transition-colors hover:text-indigo-400"
      ...
      <div
        className="mx-auto flex max-w-7xl flex-col gap-3 px-5 py-6 text-xs sm:flex-row sm:items-center sm:justify-between lg:px-8"
        style={{ borderTop: "1px solid var(--line)", color: "var(--muted-fg)" }}
      >
...
function FooterColumn({ title, links }: { title: string; links: string[][] }) {
  return (
    <div>
      <h2 className="text-xs font-semibold uppercase tracking-[.18em]" style={{ color: "var(--muted-fg)", opacity: 0.7 }}>{title}</h2>
      <ul className="mt-5 space-y-3">
        {links.map(([label, href]) => (
          <li key={href + label}>
            <Link href={href} className="text-sm transition-colors hover:text-indigo-400" style={{ color: "var(--muted-fg)" }}>
              {label}
            </Link>
          </li>
        ))}
...
function TrustItem({ icon, title, text }: { icon: React.ReactNode; title: string; text: string }) {
  return (
    <div
      className="glass-card rounded-xl p-5 transition-all hover:-translate-y-0.5"
